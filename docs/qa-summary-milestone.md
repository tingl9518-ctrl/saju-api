...
| **야자시 정책 차이** | **1** | 자시대·날짜 경계에서 **일주·시주** 귀속 규칙이 위키와 다름 (예: 양력 2001-01-01 23:40 전후) |
| **입춘 mismatch** | **2** | **월주(절입)** 가 위키와 다름 — 번들 절입 시각 vs 국내 체감(운세위키) 차이로 해석 (예: 1995 윤8/15, 2001·2024 입춘 주변 등 레퍼런스 축) |
...
| 영역 | 내용 |
|------|------|
| **Domestic 입춘·절입 reference bundle** | `lunar-javascript` 기반 번들과 국내 만세력·운세위키 **입춘·절기 시각**이 연도별로 수 시간 벌어지는 샘플이 존재한다. **국내 reference를 source of truth**로 두는 전략이 타당하다 (`docs/jieqi-strategy.md`). |
| **야자시 정책** | 위키는 **晚子 시 일주를 익일로 묶는** 등 표현이 있고, 현재 엔진은 **양력 달력 일자 + 한국형 시진**으로 일주·시주를 맞춘다. **정책 확정 전**에는 골든에서 `known_mismatch`로만 추적한다. |
...
3. 야자시 정책이 확정되면 `known_mismatch` 케이스를 **활성 회귀로 승격**  
